...
def dashboard():
    ...
    shared_permissions = supabase.table("file_permissions") \
        .select("*").eq("shared_with", user).execute()
...
def share(filename):
    ...
    for entry in users_list:
        ...
        print("Sharing with:", u)

        existing = supabase.table("file_permissions") \
            .select("*") \
        ...
        if not existing.data:
            ...
            print("Inserting permission row")

            result = supabase.table("file_permissions").insert({
                "filename": filename,
                "owner": session["user"],
                "shared_with": u,
...
def delete(filename):
    ...
    supabase.storage.from_("encrypted-files").remove(
        [f"{user}/{filename}"]
    )
    ...
    supabase.table("file_permissions") \
        .delete() \
        .eq("filename", filename) \
        .eq("owner", user) \
        .execute()
